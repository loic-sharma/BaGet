...
public class TableSearchService : ISearchService
{
    ...
    public TableSearchService(CloudTableClient client, ISearchResponseBuilder responseBuilder)
    {
    ...
    private async Task<List<PackageRegistration>> SearchAsync(string searchText, int skip, int take, bool includePrerelease, bool includeSemVer2, CancellationToken cancellationToken)
    {
    ...
    private async Task<IReadOnlyList<Package>> LoadPackagesAsync(TableQuery<PackageEntity> query, int maxPartitions, CancellationToken cancellationToken)
    {
    ...
    private string GenerateSearchFilter(string searchText, bool includePrerelease, bool includeSemVer2)
    {
        ...
        result = GenerateAnd(result, GenerateIsTrue(nameof(PackageEntity.Listed)));
        ...
        if (!includeSemVer2)
        {
            result = GenerateAnd(result, TableQuery.GenerateFilterConditionForInt(nameof(PackageEntity.SemVerLevel), QueryComparisons.Equal, 0));
        }
