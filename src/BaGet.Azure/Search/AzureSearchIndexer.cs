...
public class AzureSearchIndexer : ISearchIndexer
{
    ...
    public AzureSearchIndexer(IPackageDatabase packages, IndexActionBuilder actionBuilder, AzureSearchBatchIndexer batchIndexer, ILogger<AzureSearchIndexer> logger)
    {
    ...
    public async Task IndexAsync(Package package, CancellationToken cancellationToken)
    {
        ...
        var actions = _actionBuilder.UpdatePackage(new PackageRegistration(package.Id, packages));
