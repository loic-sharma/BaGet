...
{
    public class RegistrationIndexService : IRegistrationIndexService
    {
        private readonly IMirrorService _mirror;
        private readonly IPackageService _packages;

        public RegistrationIndexService(IMirrorService mirror, IPackageService packages)
        {
            _mirror = mirror ?? throw new ArgumentNullException(nameof(mirror));
            _packages = packages ?? throw new ArgumentNullException(nameof(packages));
        }
        private RegistrationIndexPageItem ToRegistrationIndexPageItem(Package package, IUrlExtension Url) =>
        ...
        public async Task<RegistrationIndex> Get(string id, IUrlExtension Url, CancellationToken cancellationToken)
        {
            ...
            return new RegistrationIndex(
                ...
                pages: new[]
                {
                    new RegistrationIndexPage(
                        ...
                        count: packages.Count(),
                        itemsOrNull: packages.Select( (p) => ToRegistrationIndexPageItem(p,Url)).ToList(),
                        lower: versions.Min(),
                        upper: versions.Max())
                });
        }
