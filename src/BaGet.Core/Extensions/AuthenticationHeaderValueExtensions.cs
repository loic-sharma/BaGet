...
namespace BaGet.Core.Extensions
{
    public class AuthenticationHeaderValueExtensions
    {
        ...
        public static string GetBasicAuthHeaderString(NetworkCredential credential)
        {
            ...
            {
                username = string.Format("{0}@{1}", credential.UserName, credential.Domain);
            }
            return string.Format("{0}:{1}", username, credential.Password);
        }
        ...
        public static bool TryGetNetworkCredentialFromString(string credentialString, out NetworkCredential credential)
        {
            ...
            credential = null;
            int firstColon = credentialString.IndexOf(':');
            if (firstColon < 0) return false;
            string userPart = credentialString.Substring(0, firstColon);
            string password = credentialString.Substring(firstColon+1);
            var username = userPart;
            string domain = null;
            int firstAt = userPart.IndexOf('@');
            if (firstAt > -1)
            {
